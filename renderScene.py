...
in  float alpha, in float c1, out vec3 reflected, out vec3 refracted) {
 ...
 vec3 water_decay(vec3 color, float distance) {
    float mask=exp(-distance*u_depth_mult/2);        //ADDITIONALS
    return mix(u_water_ambient_color, color, mask);
}
 vec3 water_depth_mix(vec3 color, vec3 position) {  //ADDITIONALS
    float mask=1/exp(position.z*3);                 //ADDITIONALS
    return mix(u_water_depth_color, color, mask);   //ADDITIONALS
}
...
void main() {
    ...
    // compute reflection and refraction
     ...
     float reflectance;
     float path_in_water;
    if(c>0.0) { // looking from air to water    //ADDITIONALS
    ...
    } else { // looking from water to air               //ADDITIONALS /***/
        ...
        bed_texcoord=get_bed_texcoord(point_on_bed);                //
        path_in_water=length(point_on_bed-v_position); //ADDITIONALS /***/
    };
    // fetch texture
    ...
    } else { // under water                            //ADDITIONALS
        sky+=sun_contribution(refracted, normal);      //ADDITIONALS
        vec3 bed=water_decay(bed_color*u_bed_mult, path_in_water);   //ADDITIONALS
        rgb=water_decay(mix(sky, bed, reflectance),distance_to_eye); //ADDITIONALS
    };
...
class Canvas(app.Canvas):
    def __init__(self, surface, sky="textures/2.png", bed="textures/1.png"):
        self.surface = surface
        self.sky = io.read_png(sky)
        self.bed = io.read_png(bed)
        app.Canvas.__init__(self, size=(800, 600), title="OpenGL Water Simulation")
        ...
        self.program["u_water_ambient_color"] = [0.1, 0.4, 0.4]
        self.program["u_water_depth_color"] = [0, 0.1, 0.1] #ADDITIONALS
...
if __name__ == '__main__':
    ...
    surface = CircularWaves(size=(100, 100), max_height=0.02)
    c = Canvas(surface)
    c.measure_fps()
    app.run()
